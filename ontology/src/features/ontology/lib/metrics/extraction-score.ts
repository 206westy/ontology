...
import type { RelationLayer } from '../schemas';

const RELATION_LAYERS: RelationLayer[] = ['semantic', 'kinetic'];
...
export interface ScoredRelation {
  ...
  target: string;
  type: string;
  // PRD-L M2: layer 실측용. 없으면 layer 채점에서 제외(회귀 안전).
  layer?: RelationLayer;
}
...
// 2×2 혼동행렬: confusion[expected][actual] = 건수. 어느 레이어가 어디로
// 오분류되는지(semantic↔kinetic) 가시화한다.
export type LayerConfusion = Record<
  RelationLayer,
  Record<RelationLayer, number>
>;

export interface LayerScore {
  // (source,target,type)로 매칭된 관계 중 layer가 일치하는 비율.
  // 양쪽 모두 layer 라벨이 있는 매칭이 하나도 없으면 null.
  accuracy: number | null;
  matched: number; // layer 라벨이 양쪽에 있는 매칭 수
  correct: number; // 그중 layer 일치 수
  confusion: LayerConfusion;
}
...
function emptyConfusion(): LayerConfusion {
  const m = {} as LayerConfusion;
  for (const e of RELATION_LAYERS) {
    m[e] = {} as Record<RelationLayer, number>;
    for (const a of RELATION_LAYERS) m[e][a] = 0;
  }
  return m;
}
...
export function scoreExtraction(expected: ScoredSet, actual: ScoredSet): ExtractionScore {
  ...
  // Layer 채점: (source,target,type)로 매칭된 관계 중 layer 일치율 + 혼동행렬.
  const expectedLayerByKey = new Map<string, RelationLayer>();
  for (const r of expected.relations) {
    if (r.layer) expectedLayerByKey.set(relationKey(r), r.layer);
  }
  const actualLayerByKey = new Map<string, RelationLayer>();
  for (const r of actual.relations) {
    if (r.layer) actualLayerByKey.set(relationKey(r), r.layer);
  }
  const confusion = emptyConfusion();
  let matched = 0;
  let correct = 0;
  for (const [key, expLayer] of expectedLayerByKey) {
    const actLayer = actualLayerByKey.get(key);
    if (!actLayer) continue;
    matched += 1;
    confusion[expLayer][actLayer] += 1;
    if (expLayer === actLayer) correct += 1;
  }
