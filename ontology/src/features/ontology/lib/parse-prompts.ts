...
Attributes, modes, states, and measurements are PROPERTIES — never nodes (L5, unified rule):
- An entity's attribute, measurement, operating MODE, STATE, or selectable OPTION (weight, rated power, temperature, part number; "RAG 모드"/"Agent 모드", "가동"/"정지", "Auto"/"Manual") is a PROPERTY of the entity it belongs to — NEVER its own class or instance node. Do not emit a separate entity for the attribute/mode NAME (무게, 정격 출력, mode) nor for its VALUE (999그램, 5kW, RAG).
- A categorical value (a fixed set of mode/state/option labels) becomes an enum property: { name: "mode", value: "RAG", dataType: "enum", enumValues: ["RAG", "Agent"] }. Other attributes use their natural dataType. Attach it to the owning instance's properties.
...
- COMMIT CONFIDENTLY to instance when the entity carries an individual identifier — a proper noun (a person's name), a unit/machine number (장비 호기), or a part/serial number. These name one specific thing, not a kind; do not hedge them into classes.
- Property DEFINITIONS (e.g. partNumber: string) live on the class; the actual VALUE (e.g. KC0330655) lives on the instance — put it in that instance's properties as { name: "partNumber", value: "KC0330655", dataType: "string" }. (Attributes/measurements are never their own node — see the unified rule above.)
